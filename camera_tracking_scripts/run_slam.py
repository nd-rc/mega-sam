...
# === НАЧАЛО HACK ДЛЯ RTX 5070 (Без UniDepth) ===
  
  # 1. Находим файлы глубины (ищем .npy от DepthAnything)
  import glob
  mono_disp_paths = sorted(glob.glob(os.path.join(args.mono_depth_path, "*.npy")))
  if len(mono_disp_paths) == 0:
      # Если вдруг там картинки png
      mono_disp_paths = sorted(glob.glob(os.path.join(args.mono_depth_path, "*.png")))
  
  print(f"HACK: Found {len(mono_disp_paths)} depth files in {args.mono_depth_path}")

  # 2. Загружаем первую картинку для размеров
  ...
  # 4. Загружаем глубину в список (без выравнивания)
  mono_disp_list = []
  for f in mono_disp_paths:
      d = np.load(f) # Загружаем npy
      # Если размер не совпадает - ресайзим
      if d.shape[:2] != (H, W):
          d = cv2.resize(d, (W, H), interpolation=cv2.INTER_NEAREST)
      mono_disp_list.append(d)

  # 5. Фиктивные параметры выравнивания (так как метрики нет)
  align_scale = 1.0
  align_shift = 0.0
  
  # Нормализация, чтобы числа не были слишком огромными/маленькими
  if len(mono_disp_list) > 0:
      # Берем каждый 10-й кадр для скорости
      stack = np.array(mono_disp_list[::10]) 
      normalize_scale = np.percentile(stack, 98) / 2.0
  else:
      normalize_scale = 1.0
